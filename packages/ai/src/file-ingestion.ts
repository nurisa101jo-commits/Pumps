...
export async function parseIngestionFile(input:IngestionFile,ocr?:ImageOcrProvider):Promise<ParsedDocument>{
 ...
 if(m==="application/pdf"||input.fileName.toLowerCase().endsWith(".pdf")){const result=await pdfParse(b);return{documentId:input.documentId,fileName:input.fileName,mimeType:input.mimeType,pages:[{text:result.text}],rawText:result.text}}
 if(m.includes("spreadsheet")||m.includes("excel")||/\.(xlsx|xls|csv)$/i.test(input.fileName)){const wb=XLSX.read(b,{type:"buffer",cellDates:false});const pages=wb.SheetNames.map(name=>({table:name,text:XLSX.utils.sheet_to_csv(wb.Sheets[name])}));return{documentId:input.documentId,fileName:input.fileName,mimeType:input.mimeType,pages,rawText:pages.map(x=>"["+x.table+"]\n"+x.text).join("\n")}}
