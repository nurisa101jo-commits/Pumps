...
export function undoLastAudit(store:CatalogStore,entityType:string,entityId:string){const events=store.audit.filter(x=>x.entityType===entityType&&x.entityId===entityId);const last=events.at(-1);if(!last||last.before===undefined)throw new Error("No reversible audit event found");if(entityType==="pump_configuration"){store.configurations.set(entityId,last.before as any)}else if(entityType==="pump_model"){store.models.set(entityId,last.before as any)}else if(entityType==="pump_series"){store.series.set(entityId,last.before as any)}else if(entityType==="motor"){store.motors.set(entityId,last.before as any)}else throw new Error("Entity type is not reversible");return last}
