...
export async function loadCatalog(store:CatalogStore){
 ...
 const auditRows=(await q.query("SELECT id,entity_type AS \"entityType\",entity_id AS \"entityId\",action,actor_id AS \"actorId\",timestamp,before_json,after_json FROM audit_events ORDER BY timestamp,id")).rows;store.audit=auditRows.map((x:any)=>({...x,before:x.before_json?JSON.parse(x.before_json):undefined,after:x.after_json?JSON.parse(x.after_json):undefined,metadata:x.metadata_json?JSON.parse(x.metadata_json):undefined}));
