...
async function publishCatalogPayload(ingestion:IngestionStore,catalog:CatalogStore,engineering:EngineeringOptionStore,candidate:any,publishedBy:string,linkSource:(entityType:string,entityId:string,fieldName:string,source:SourceReference)=>Promise<void>){
 ...
 if(catalog.pool){
  ...
  try{
   ...
   for(const c of created.configurations)await client.query("INSERT INTO pump_configurations(id,model_id,code,motor_id,seal,connection,materials_json,weight_kg,active) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9)",[c.id,c.modelId,c.code,c.motorId,c.seal??null,c.connection??null,JSON.stringify(c.materials??{}),created.dimensions.find((d:any)=>d.configurationId===c.id)?.weightKg??null,c.active]);
