...
export type ProjectSelection={id:string;projectId:string;configurationId:string;selectedOptionIds:string[];createdAt:string;engineVersion?:string;dutyResults?:Array<{q:number;head:number;availableHead:number;efficiency?:number;npshr?:number;powerKw?:number;motorLoadRatio?:number}>;warnings?:string[]};
