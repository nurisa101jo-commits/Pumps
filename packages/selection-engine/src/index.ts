...
export function selectFromCatalog(request:SelectionRequest,catalog:CatalogConfiguration[]):SelectionResult{
 ...
 const candidates=catalog.flatMap(c=>{
  ...
  if(request.constraints?.maxMotorLoadRatio!==undefined){if(dutyResults.some(x=>x.motorLoadRatio===undefined)){warnings.push(`Configuration ${c.id} cannot be checked against max motor load ratio because no structured absorbed-power curve is available`);return []}if(dutyResults.some(x=>x.motorLoadRatio!>request.constraints!.maxMotorLoadRatio!)){warnings.push(`Configuration ${c.id} rejected because absorbed power exceeds the permitted motor load ratio`);return []}}
