...
export function evaluateRules(context:ConfigurationContext,rules:EngineeringRule[]):RuleEvaluation[]{
 return rules.filter(r=>r.enabled&&(!r.configurationIds||r.configurationIds.includes(context.configuration.id))).map(rule=>{
  ...
  switch(rule.kind){
   ...
   case"fluid_compatibility":if(Array.isArray(p.allowedFluids)&&context.fluidName&&!p.allowedFluids.includes(context.fluidName)){passed=false;message=`Fluid ${context.fluidName} is not permitted`}break;
   ...
   case"application":if(Array.isArray(p.allowedApplications)&&context.application&&!p.allowedApplications.includes(context.application)){passed=false;message=`Application ${context.application} is not permitted`}break;
  }
