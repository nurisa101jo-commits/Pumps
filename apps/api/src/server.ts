...
async function start(){
 if(pool){await pingDatabase(pool);await loadCatalog(catalogStore);databaseReady=true}
 registerCatalogRoutes(app,catalogStore);
 registerProjectRoutes(app,projectStore);
 app.get("/health",async()=>({status:"ok",service:"pumps-api",version:"0.4.0",database:pool?(databaseReady?"ready":"not-ready"):"memory"}));
