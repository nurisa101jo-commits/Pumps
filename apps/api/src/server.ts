...
import {ensureDocument} from "@pumps/db/document-service";
...
app.post("/api/v1/ingestion/candidates/:candidateId/publish",async(req,reply)=>{try{return reply.code(201).send(await publishCandidate(ingestionStore,publicationStore,catalogStore,documentStore,engineeringStore,(req.params as any).candidateId,req.body as any))}catch(e){return reply.code(409).send({error:e instanceof Error?e.message:"Publication rejected"})}});
...
app.post("/api/v1/backups",async(req,reply)=>{try{return reply.code(201).send(await createCatalogBackup(catalogStore,((req.body as any)?.type==="automatic"?"automatic":"manual")))}catch(e){return reply.code(500).send({error:e instanceof Error?e.message:"Backup failed"})}});
 ...
 await app.listen({host:"0.0.0.0",port:Number(process.env.PORT??4000)}); const backupHours=Number(process.env.AUTOMATIC_BACKUP_HOURS??0); if(backupHours>0)setInterval(()=>createCatalogBackup(catalogStore,"automatic").catch(error=>app.log.error(error,"Automatic backup failed")),backupHours*3600000);
}
