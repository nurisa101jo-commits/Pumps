...
export function registerProjectRoutes(app:FastifyInstance,store=createProjectStore(),catalog:CatalogConfiguration[]|(()=>CatalogConfiguration[])=[]){
  ...
  app.post("/api/v1/projects/:projectId/selection",async(req,reply)=>{
    try{
      const projectId=(req.params as any).projectId;
      const body=req.body as any; const selection:ProjectSelection={id:makeId(),projectId,createdAt:new Date().toISOString(),...(body)}; const points=[...store.dutyPoints.values()].filter(x=>x.projectId===projectId); const result=selectFromCatalog({...body.request,dutyPoints:body.request?.dutyPoints??points.map(x=>({q:x.q,head:x.head,label:x.label})),},getCatalog()); const candidate=result.candidates.find(x=>x.configurationId===selection.configurationId); if(!candidate)throw new Error("Selected configuration is not valid for the project duty points"); const config=getCatalog().find(x=>x.id===selection.configurationId)!; const selectedIds=selection.selectedOptionIds??[];const allowedByKind=config.optionIds??{};const allowed=new Set(Object.values(allowedByKind).flat());for(const optionId of selectedIds)if(!allowed.has(optionId))throw new Error("Selected option is not compatible with the configuration");for(const [kind,ids] of Object.entries(allowedByKind)){if(kind!=="material"&&selectedIds.filter(x=>ids?.includes(x)).length>1)throw new Error("Only one option is allowed for kind: "+kind)} selection.engineVersion=result.engineVersion;selection.dutyResults=candidate.dutyResults.map((point:any)=>({q:point.q,head:point.requiredHead,availableHead:point.availableHead,...(point.efficiency!==undefined?{efficiency:point.efficiency}:{}),...(point.npshr!==undefined?{npshr:point.npshr}:{}),...(point.powerKw!==undefined?{powerKw:point.powerKw}:{}),...(point.motorLoadRatio!==undefined?{motorLoadRatio:point.motorLoadRatio}:{})}));selection.warnings=result.warnings;
