...
app.post("/api/v1/catalog/configurations/:configurationId/publish",{preHandler:engineer},async(req,reply)=>{try{const id=(req.params as any).configurationId;const item=store.configurations.get(id);if(!item)return reply.code(404).send({error:"Configuration not found"});if(item.active===false)return reply.code(400).send({error:"Inactive configuration cannot be published"});const event={id:makeId(),entityType:"configuration",entityId:id,action:"publish_from_ingestion" as const,actorId:actor(req),timestamp:new Date().toISOString(),after:item};await recordAuditPersistent(store,event);return {published:true,configurationId:id,publishedAt:event.timestamp}}catch(e){return reply.code(400).send({error:e instanceof Error?e.message:"Publication failed"})}});
