...
app.get("/api/v1/catalog/configurations",async(req)=>{const modelId=(req.query as any)?.modelId as string|undefined;return[...store.configurations.values()].filter(x=>!modelId||x.modelId===modelId).map(x=>({...x,model:store.models.get(x.modelId)??null,motor:store.motors.get(x.motorId)??null,dimensions:[...store.dimensions.values()].find(d=>d.configurationId===x.id)??null,curves:[...store.curves.values()].filter(c=>c.configurationId===x.id)}))});
app.get("/api/v1/catalog/configurations/:configurationId",async(req,reply)=>{const id=(req.params as any).configurationId;const x=store.configurations.get(id);if(!x)return reply.code(404).send({error:"Configuration not found"});return{...x,model:store.models.get(x.modelId)??null,motor:store.motors.get(x.motorId)??null,dimensions:[...store.dimensions.values()].find(d=>d.configurationId===x.id)??null,curves:[...store.curves.values()].filter(c=>c.configurationId===x.id)}});
