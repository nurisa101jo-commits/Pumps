...
app.post("/api/v1/catalog/motors",async(req,reply)=>{const x=req.body as any;if(!Number.isFinite(x.powerKw)||x.powerKw<=0)return reply.code(400).send({error:"Motor powerKw must be positive"});if(x.voltageV!==undefined&&(!Number.isFinite(x.voltageV)||x.voltageV<=0))return reply.code(400).send({error:"Voltage must be positive"});if(x.phase!==undefined&&!([1,3].includes(Number(x.phase))))return reply.code(400).send({error:"Phase must be 1 or 3"});const item={id:makeId(),...x};store.motors.set(item.id,item);await persistCatalogItem(store,"motor",item);return reply.code(201).send(item)});
...
app.post("/api/v1/catalog/configurations",async(req,reply)=>{try{const x=req.body as any;validateConfiguration(store,x);if(engineeringStore)for(const option of (x.optionIds??[])){if(!["material","seal","connection","impeller","accessory"].includes(option.kind))throw new Error("Invalid option kind");if(!engineeringStore.options.get(option.kind)?.has(option.id))throw new Error("Option does not exist for kind: "+option.kind)}const item={id:makeId(),...x};assertUnique(store,"configurations","code",item.code);store.configurations.set(item.id,item);await persistCatalogItem(store,"configuration",item);if(engineeringStore)for(const option of (x.optionIds??[])){await linkConfigurationOption(engineeringStore,item.id,option.kind as EngineeringOptionKind,option.id)}return reply.code(201).send(item)}catch(e){return reply.code(400).send({error:e instanceof Error?e.message:"Invalid configuration"})}});
